...
import numpy
import numpy.linalg
import stlib3.splib.numerics.Quaternion
from stlib3.splib.numerics.Quaternion import from_euler, to_matrix
from math import pi
from stlib3.splib.numerics.vec3 import *
from stlib3.splib.numerics.quat import *
from stlib3.splib.numerics.matrix import *
...
class RigidDof(object):
    ...
    def setPosition(self, v, field="position"):
        ...
        if(not isinstance(v,list)):
            v = list(v)
        print(type(p.value[0][3:]),v,[float(i) for i in p.value[0][3:]])
        tmp = numpy.append(v,p.value[0][3:])
        print(tmp)
        p.value = [tmp,tmp]
